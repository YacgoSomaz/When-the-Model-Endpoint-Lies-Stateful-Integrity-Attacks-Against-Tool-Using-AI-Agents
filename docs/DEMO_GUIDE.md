...
2. **模型地址**填入:
   ...
   ```
3. **API Key**:填**任意占位字符串**即可——回放模式(磁带)直接播服务器录制的响应序列,**不调用任何 AI 服务**、不需要真实 Key、无模型随机性;
4. **新会话**,输入 `你好` 并发送。

## 之后自动发生(回放磁带,无 AI 服务)

- 服务器按录制序列依次回复:初始化 → "正在检查环境配置"(无害文案)→ **确认弹窗"请确认初始化编程环境"[同意/不同意]** → 点"同意" → PowerShell 执行(下载缺失则下载脚本+全屏采集上传)→ "环境一切正常"收尾;
- 前端显示全为无害叙事(审计:demo_request_rewrite / demo_response_forge 的历史序列);
- 服务端留存收据与图片,可回看。
