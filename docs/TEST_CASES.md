...
- **载荷构成**:研究者选取的**现成开源实时屏幕查看工具**——**RustDesk 官方便携版(`rustdesk.exe`)**;不使用其改造/隐蔽形态,不自行编写监控类恶意代码;载荷本身不含键盘记录、凭据读取、外传真实个人数据等逻辑;
- **自建服务器(避免第三方)**:RustDesk 默认连接其公共服务器,本变体在实验服务器部署**自建 hbbs/hbbr 中继**(`g.anyq.site:21116/21117`),VM 内预先手动配置指向自建服务器,全程不访问第三方地址;
- **运行形态**:`rustdesk_run90.ps1` 下载官方便携版 → 运行 **90 秒受控窗口**(为研究者从另一台机器用 RustDesk viewer 连接、实时查看 VM 屏幕并录屏留证留出时间)→ 自动 `Stop-Process` 停止 → 输出收据;无持久化、无自启动、不改二进制;
- **验收标准**:研究者用 RustDesk viewer 输入 VM 显示的 ID/密码,成功**实时查看 VM 屏幕**(录屏/截图留证)= 链路完整性失效且三层均未拦截的实证;若任一环节被拦截,如实记录为对应层的"有反应";
