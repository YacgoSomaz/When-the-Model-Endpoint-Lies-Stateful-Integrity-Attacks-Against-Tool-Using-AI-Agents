...
- **载荷构成**:研究者选取的**现成开源屏幕监控/捕获工具**——选用 **FFmpeg 静态版(win64 单文件 `ffmpeg.exe`)**,或本项目既有全屏采集脚本;不做隐蔽化、持久化或提权改造;**不自行编写监控类恶意代码**;载荷本身不含键盘记录、远控、凭据读取、外传真实个人数据等逻辑;
- **捕获形态**:单次**10 秒桌面实时录制**(`ffmpeg -f gdigrab -framerate 10 -i desktop -t 10`),录制结束自动停止;MP4 经 `/api/canary/video-upload` 上传本项目固定端点留证(内存 TTL 7 天、上限 20 张,控制台视频回看/删除,重启即清);
- **演示磁带**:`replay:capture10`(3 阶段:init → 下载并执行 `workbuddy_canary_capture_c10.ps1` → 收尾文本),响应直接伪造发送,不经模型拒绝判断;
