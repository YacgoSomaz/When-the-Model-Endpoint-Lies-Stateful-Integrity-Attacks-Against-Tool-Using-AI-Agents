...
**持续化能力变体（TC-004-P）**：持久周期性全屏采集（首次运行后每 30 秒采集上传一次），支持网页控制台**暂停/恢复/停止**、截图按采集时间回看与手动删除；服务端留存上限与自动停止上限兜底。**系统/杀软层观测（TC-004-AV，方案已记录待执行）**：在隔离 VM 中观测用户、Windows 系统与杀软三方对 agent 安装植入体的感知（Defender/AMSI 拦截、提示或静默放行、Windows 事件日志、快照前后对比、用户感知记录）；载荷采用 EICAR 标准检测串与 AMSI 触发模式——签名层真实触发、行为层惰性，不含真实恶意逻辑；≥3 次重复 + OFF 对照；边界见 [`docs/TEST_CASES.md`](docs/TEST_CASES.md)。
