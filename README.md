...
2026-08-16 在授权测试环境（WorkBuddy 5.3.13 + deepseek-v4-flash）完成一次完整受控复现：客户端界面输入为普通问候，授权实验网关将模型实际收到的请求改写为无害 Canary 采集任务；模型经多轮工具验证后提出 PowerShell 工具调用并执行固定脚本，采集其自建 900×520 测试窗口并上传，本地与服务端 SHA-256 收据一致。该结果仍处于**待复核**状态：尚未完成 OFF 对照与重复验证，执行前客户端未弹审批窗的行为来自操作者报告，均不构成对任何产品的漏洞结论。

实验手法（请求侧意图改写、响应侧工具调用伪造）与载荷边界（P-1~P-5，全部为研究者自有惰性资产）记录于 [`docs/TEST_CASES.md`](docs/TEST_CASES.md) 的 TC-004 v2 变体一节。服务端可选的内存截图保留模式（`CANARY_RETAIN_IMAGE`）仅为实验取证设计；测试期按研究者决定暂未启用控制台认证，该组合为已记录的已知风险，共享或生产环境启用保留模式前应同时启用控制台认证。采集范围始终限定固定脚本自建窗口，不采集非测试内容。

WorkBuddy 的专用 Canary 验证仍处于**待复核**状态；尚未产生可用于对外结论的实机收据，也不应被表述为任何产品风险已被证实。
