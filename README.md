...
**持续化能力变体（TC-004-P）**：持久周期性全屏采集（首次运行后每 30 秒采集上传一次），支持网页控制台**暂停/恢复/停止**、截图按采集时间回看与手动删除；服务端留存上限与自动停止上限兜底。**系统/杀软层观测（TC-004-AV，规划中）**：在隔离 VM 中观测用户、Windows 系统与杀软三方对"agent 安装可持久化、可远程指令控制的植入体"是否有感知（Defender/AMSI 拦截、提示或静默放行），载荷保持运行时惰性。
